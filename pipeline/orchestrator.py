"""Enhanced orchestrator with Telegram notifications when available.

This file replaces pipeline/orchestrator.py to add notification support.
"""
import os
import subprocess
import json
...
def process_job(prompt: str, platforms: list[str], aspect: str = "9:16"):
    ...
    try:
        ...
        total_duration = int(sum([s.get('duration',3) for s in scenes]))
        _run(["python", "scripts/detect_hooks.py", "--video", str(final_out), "--duration", str(total_duration), "--out", str(hooks_json)])
        ...
        for p in platforms:
            ...
            if up_script.exists():
                try:
                    ...
                    uploads.append({"platform": p, "status": f"upload failed: {e}"})
            else:
        ...
        # Send notification
        try:
            message = f"✅ Job <b>{run_id}</b> complete.\nPrompt: {prompt}\nOutput: {final_out}\nUploads: {uploads}"
        ...
        # notify failure
        try:
            send_telegram(f"❌ Job <b>{run_id}</b> failed. Error: {err}")
        except Exception:
            pass
        raise
